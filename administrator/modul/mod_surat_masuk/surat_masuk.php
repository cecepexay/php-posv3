...
<script language="JavaScript" type="text/JavaScript">
 ...
 while ($data = mysql_fetch_array($hasil))
 {
   ...
   echo "if (document.form_surat_masuk.id_kategori.value == \"".$idkategori."\")";
   echo "{";
 ...
 while ($data1 = mysql_fetch_array($hasil1))
 {
   ...
   // membuat IF untuk masing-masing kategori
   echo "if (document.form_surat_masuk_pegawai.id_kategori.value == \"".$idkategori."\")";
   echo "{";
...
$aksi="modul/mod_surat_masuk/aksi_surat_masuk.php";
switch($_GET[act]){
  // Tampil kategori
  default:
    ...
    if ($_SESSION[leveluser]=='admin'){
        ...
        $tampil_surat_masuk = mysql_query("SELECT * FROM surat_masuk ORDER BY tgl_surat DESC");
        $cek_surat_masuk = mysql_num_rows($tampil_surat_masuk);
        if(!empty($cek_surat_masuk)){
        echo "<h2>Manajemen Surat Masuk</h2><hr>
          <input class='btn btn-info' type=button value='Surat Masuk' onclick=\"window.location.href='?module=surat_masuk&act=tambahsurat_masuk';\">";
        echo "<br><br><table id='sample-table-2' class='table table-striped table-bordered table-hover'><thead>";
		if (empty($_GET['kata'])){
		echo "
          <tr><th>No</th><th>No Surat</th><th>Judul</th><th>Tgl Surat</th><th>Tgl Terima</th><th>Kategori</th><th>Rak</th><th>File</th><th>Tgl Entry</th><th>Pembuat</th><th>Deskripsi</th><th>Aksi</th></tr></thead>";
       $no=1;
    while ($r=mysql_fetch_array($tampil_surat_masuk)){
	  ...
	  $tgl_terima   = tgl_indo($r[tgl_terima]);
       echo "<tr><td>$no</td>
             ...
             echo"
			 <td>$r[deskripsi]</td>
             <td><a href='?module=surat_masuk&act=editsurat_masuk&id=$r[id_file]' title='Edit'><img src='images/icons/edit.png' alt='Edit' /></a><a href=javascript:confirmdelete('$aksi?module=surat_masuk&act=hapus&id=$r[id_file]') title='Hapus'><img src='images/icons/cross.png' alt='Delete' /></a></td></tr>";
      $no++;
    }
    echo "</table>";
	...
	echo "<div class='right'><form method=get action='$_SERVER[PHP_SELF]'>
          <input type=hidden name=module value=surat_masuk>
          Cari Judul : <input type=text name='kata' size=60> <input type=submit value=Cari>
          </form></div>";
	
    echo "<tr><th>No</th><th>No Surat</th><th>Judul</th><th>Tanggal Surat</th><th>Tgl Terima</th><th>Kategori</th><th>Rak</th><th>Nama File</th><th>Tgl Entry</th><th>Pembuat</th><th>Deskripsi</th><th>Aksi</th></tr></thead>";


    if ($_SESSION[leveluser]=='admin'){
      $tampil = mysql_query("SELECT * FROM surat_masuk WHERE judul LIKE '%$_GET[kata]%' ORDER BY tgl_surat DESC");
    }

  else{
      $tampil=mysql_query("SELECT * FROM surat_masuk 
    ...
    if ($_SESSION[leveluser]=='admin'){
      $jmldata = mysql_num_rows(mysql_query("SELECT * FROM surat_masuk WHERE judul LIKE '%$_GET[kata]%'"));
    }
    else{
      $jmldata = mysql_num_rows(mysql_query("SELECT * FROM surat_masuk WHERE username='$_SESSION[namauser]' AND judul LIKE '%$_GET[kata]%'"));
    }  
 ...
 elseif ($_SESSION[leveluser]=='pegawai'){        
     ...
     $tampil_surat_masuk = mysql_query("SELECT * FROM surat_masuk ORDER BY tgl_surat DESC");
		...
		echo "
          ...
          <tr><th>No</th><th>No Surat</th><th>Judul</th><th>Tanggal Surat</th><th>Tgl Terima</th><th>Kategori</th><th>Rak</th><th>Nama File</th><th>Tgl Entry</th><th>Pembuat</th><th>Deskripsi</th><th>Aksi</th></tr></thead>";
       $no=1;
    while ($r=mysql_fetch_array($tampil_surat_masuk)){
       ...
       echo "<table>
        ...
        echo "<tr><td>$no</td>
             ...
             echo "<td><input type=button class='tombol' value='Lihat File Materi'
                       onclick=\"window.location.href='?module=surat_masuk&act=daftarsurat_masuk&id=$r[id_rak]';\"></td></tr>";
        $no++;
        }
        echo "</table>";
...
case "tambahsurat_masuk":
    if ($_SESSION[leveluser]=='admin'){
    echo "<form name='form_surat_masuk' method=POST action='$aksi?module=surat_masuk&act=input_surat_masuk' enctype='multipart/form-data'>
     <fieldset>
    ...
    echo "</dd>
	<dt><label>Tanggal Terima</label></dt><dd>  ";
          combotgl(1,31,'tgl1',$tgl_skrg);
          combonamabln(1,12,'bln1',$bln_sekarang);
          combothn(1950,$thn_sekarang,'thn1',$thn_sekarang);
    ...
    echo "
    <form name='form_surat_masuk_pegawai' method=POST action='$aksi?module=surat_masuk&act=input_surat_masuk' enctype='multipart/form-data'>
    <fieldset>
    <legend>Surat Masuk</legend>
    <dl class='inline'>
	<dt><label>No Surat</label></dt>              <dd> <input type=text name='no_surat' size=50></dd>
	...
	<dt><label>Deskripsi</label></dt>             <dd>: <textarea name='deskripsi' id='wysiwyg' class='medium' rows='6'></textarea>
    <p align=center><input class='btn btn-info' type=submit value=Simpan>
                      <input class='btn btn-info' type=button value=Batal onclick=\"window.location.href='?module=surat_masuk';\"></p>
    </dl></fieldset></form>";
    }
    break;

case "editsurat_masuk":
    if ($_SESSION[leveluser]=='admin'){
    $edit=mysql_query("SELECT * FROM surat_masuk WHERE id_file = '$_GET[id]'");
    ...
    echo "
    <form name='form_surat_masuk' method=POST action='$aksi?module=surat_masuk&act=edit_surat_masuk' enctype='multipart/form-data'>
    ...
    echo "</dd>
	<dt><label>Tanggal Terima</label></dt><dd> : ";
          $get_tgl=substr("$r[tgl_terima]",8,2);
          combotgl(1,31,'tgl1',$get_tgl);
          $get_bln=substr("$r[tgl_terima]",5,2);
          combonamabln(1,12,'bln1',$get_bln);
          $get_thn=substr("$r[tgl_terima]",0,4);
          combothn(1950,$thn_sekarang,'thn1',$get_thn);
    ...
    echo "<form name='form_surat_masuk_pegawai' method=POST action='$aksi?module=surat_masuk&act=edit_surat_masuk' enctype='multipart/form-data'>
    <input type=hidden name=id value='$m[id_file]'>
    <fieldset>
    <legend>Edit Surat Masuk</legend>
    <dl class='inline'>
    ...
    echo "</dd>;
	<dt><label>Tanggal Terina</label></dt><dd> : ";
          $get_tgl=substr("$r[tgl_terima]",8,2);
          combotgl(1,31,'tgl1',$get_tgl);
          $get_bln=substr("$r[tgl_terima]",5,2);
          combonamabln(1,12,'bln1',$get_bln);
