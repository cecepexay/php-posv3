...
// Input pegawai
elseif ($module=='admin' AND $act=='input_pegawai'){
  $lokasi_file    = $_FILES['fupload']['tmp_name'];
  $tipe_file      = $_FILES['fupload']['type'];
  $nama_file      = $_FILES['fupload']['name'];
  $direktori_file = "../../../foto_pegawai/$nama_file";
  ...
  if (empty($ketemu)){
      //apabila ada foto yang di upload
        if (!empty($lokasi_file)){
            ...
            window.location=(href='../../media_admin.php?module=admin&act=tambahpegawai')</script>";
            }else{
...
elseif ($module=='admin' AND $act=='update_pegawai'){
  $lokasi_file    = $_FILES['fupload']['tmp_name'];
  $tipe_file      = $_FILES['fupload']['type'];
  $nama_file      = $_FILES['fupload']['name'];
  $direktori_file = "../../../foto_pegawai/$nama_file";
  ...
  $cek_nip = mysql_query("SELECT * FROM pegawai WHERE id_pegawai = '$_POST[id]'");
  ...
  elseif(!empty($lokasi_file) AND empty($_POST[password])){
      if (file_exists($direktori_file)){
            echo "<script>window.alert('Nama file gambar sudah ada, mohon diganti dulu');
            window.location=(href='../../media_admin.php?module=admin&act=pegawai')</script>";
            }else{
                if($tipe_file != "image/jpeg" AND
                    $tipe_file != "image/jpg"
                ){
                ...
                $cek = mysql_query("SELECT * FROM pegawai WHERE id_pegawai = '$_POST[id]'");
                $r = mysql_fetch_array($cek);
                if(!empty($r[foto])){
                $img = "../../../foto_pegawai/$r[foto]";
                unlink($img);
                $img2 = "../../../foto_pegawai/medium_$r[foto]";
                unlink($img2);
                $img3 = "../../../foto_pegawai/small_$r[foto]";
                unlink($img3);
  ...
  elseif(empty($lokasi_file) AND !empty($_POST[password])){
    ...
    header('location:../../media_admin.php?module=admin&act=pegawai');
  }
...
elseif ($module=='admin' AND $act=='update_pegawai2'){
  $lokasi_file    = $_FILES['fupload']['tmp_name'];
  $tipe_file      = $_FILES['fupload']['type'];
  $nama_file      = $_FILES['fupload']['name'];
  $direktori_file = "../../../foto_pegawai/$nama_file";
  ...
  elseif($_POST['nip']!= $ketemu['nip']){
      $cek_nip2 = mysql_query("SELECT * FROM pegawai WHERE nip = '$_POST[nip]'");
