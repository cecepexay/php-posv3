...
$login_guru =mysql_query("SELECT * FROM pegawai WHERE username_login='$username' AND password_login='$pass' AND blokir='N'");
...
elseif($ketemu_guru > 0){
  ...
  $_SESSION[nip]          = $g[nip];
  $_SESSION[idpegawai]   = $g[id_pegawai];
  ...
  mysql_query("UPDATE pegawai SET id_session='$sid_baru' WHERE username_login='$username'");
  header('location:media_admin.php?module=home');
}
else{
   echo "<link href=css/style.css rel=stylesheet type=text/css>";
